                        ...
                        private sf::Transformable
{
    ...
    public:
        //! NOTE: Those focus on the parent
        //! Parent side: allows to attach a child to this transform.
        /*!
         * If the transform is already attached, it will be detached.
         * A transform can only be attached to one transform at a time, and only be a child for one transform at a time.
         * \param child Child to attach to this transform
         */
        void attach_child( Transform* child );

        //! Parent side: allows to detach a specific child from this transform
        /*!
         * Notice that the detached child is not destroyed.
         * \param child Child to detach from the transform.
         */
        void detach_child( Transform* child );

        //! NOTE: Those focus on the child
        //! Child side: Attach the Transform to a given parent
        /*!
         *
         */
        void attach_to(Transform* parent);

        //! Child side: Detach the Transform from its parent, and returns the parent
        /*!
         *
         */
        Transform* detach();
    ...
    public:
        //! Detachs all children attached to this transform.
        void detach_children();

        //! Returns the child at index 'index', if the index is bigger that the childCount, return nullptr.
        /*!
         * \param index Index of the trasform to retreive, index starts from 0.
         * \return Transform.
         */
        Transform* get_child(int index ) const;

        //! Tells wether this transform is a child of 'parent' or not.
        /*! In other way, if 'parent' is an ancestor of this transform.
         * \param parent Target parent
         * \return True if this transform is a child of 'parent', otherwise returns false.
         */
        bool is_child_of( Transform* parent ) const;
